...
# JSON 파일 읽기
with open("restaurants_new.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
class Layout:
    ...
    def write_layout1(self):
        # 알림사항 , 자주 쓰이게 될 공간?
        st.sidebar.markdown("<h3 style='font-size: 50px; color: #2F2F4F;'>알림사항</h3>", unsafe_allow_html=True)

        col1, col2 = st.columns([1, 3])

        st.sidebar.markdown("<h3 style='font-size: 30px;'>＃간단 사용 방법</h3>", unsafe_allow_html=True)
        st.sidebar.markdown("""
            \n- 주소는 현재 지번주소로 지원합니다.
            \n- 메뉴+가격 서칭 지원합니다. 
            \n- 메뉴만 입력 가능합니다. 
            \n- 광역시도군 별 업데이트 예정입니다."
        """)
        st.sidebar.markdown("<h3 style='font-size: 30px;'>\n ps ↓</h3>", unsafe_allow_html=True)
        st.sidebar.markdown("<h3 style='font-size: 20px;'>\n 숨겨진 노포 식당을 찾습니다.</h3>", unsafe_allow_html=True)
layout = Layout()
layout.write_layout1()
